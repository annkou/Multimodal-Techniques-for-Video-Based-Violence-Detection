...
class GeminiAsyncRequester:
    ...
    async def gemini_response(
        ...
        try:
            ...
            self.save_gemini_metadata(video_path_or_url, response, metadata_json)
        except Exception as e:
            # Save error entry
            error_entry = {
                "video_path": os.path.basename(video_path_or_url)
                if not use_url
                else video_path_or_url,
                "start_time": 0.0,
                "end_time": 0.0,
    ...
    async def run_async(
        ...
        async def process_with_limit(video_path: str):
            async with sem:
    ...
    def save_gemini_metadata(self, video_path: str, response, output_json: str):
        """
